...
| Employee Data Status | `159 real employees (DEC-010), postman_code cross-validated against April 2026 reference data (DEC-011). PO supplied postman_code for 14/15 remaining new hires (DEC-012) - 129/159 now have a postman_code. 1 employee (00279833 NGUYEN VAN HIEU) intentionally left without one ("Mat tich" in source list, needs operations follow-up, not guessed). Recomputing 2026-08-26's single real data file: 112 employees resolve to a nonzero provisional piece-rate total (sum 22,778,565.15d for that one day). 40 postman codes in real raw data still unmatched - visible via /bao-cao/chua-anh-xa/.` |

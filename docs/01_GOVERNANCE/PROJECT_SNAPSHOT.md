...
| Employee Data Status | `159 real employees (DEC-010), postman_code cross-validated against April 2026 reference data (DEC-011). PO supplied postman_code for 14/15 remaining new hires (DEC-012) - 129/159 now have a postman_code. 1 employee (00279833 NGUYEN VAN HIEU) intentionally left without one ("Mat tich" in source list, needs operations follow-up, not guessed). 00279646 NGUYEN XUAN LONG marked is_active=False per PO (DEC-013). 40 postman codes in real raw data still unmatched - visible via /bao-cao/chua-anh-xa/. `position` (Chuc danh) converted 2026-08-28 from free text to a FK against a new admin-editable `PositionCatalog` (11 real values) - DEC-022.` |
| UI Status | `Sidebar-based redesign shipped 2026-08-27 (DEC-015), restructured 2026-08-28 (DEC-020, DEC-021): "Tổng quan" and "Nhân sự" pulled out of Công đoạn Phát into their own top-level sections (both shared across all công đoạn now, live in the `core` app). New Tổng quan (login landing page) shows Lương per công đoạn side by side with Năm/Tháng + Bưu cục filters. Nhân viên page (DEC-021) now groups strictly by 1 buu cuc at a time via a selector (default: first in scope) instead of mixing all offices in one list. Found + fixed a real RBAC bug along the way: payroll_detail's buu cuc dropdown would have crashed for any real Trưởng bưu cục (never caught since all testing used the admin account) - see `scope_post_office_choices()`. Khai thac dashboard fully redesigned 2026-08-28 (DEC-024): real KPI trend comparisons, vi-VN number formatting, service-mix donut, sortable/filterable/paginated daily table with mobile card fallback, sidebar unified to inline-SVG icons with desktop collapse + mobile drawer. Surfaced and fixed 2 real bugs along the way (PostOffice RBAC scoping in khaithac views, banker's-rounding in the new number filter). Same design system applied to Phát's "Lương" page 2026-08-28 (DEC-025): number-format filters promoted to shared `core/templatetags/core_extras.py`, real KPI trends, theo-buu-cuc donut, interactive employee table. Found + fixed a flexbox overflow bug affecting both pages on mobile (missing `min-width: 0`). Same design system rolled out to every remaining page 2026-08-28 (DEC-026): Tổng quan (fixed the "Bưu cục" KPI mislabel by showing it as a subtitle badge, not a KPI), Nhân viên (new Chức danh column, client-side search), Nhân viên form, Khoản hỗ trợ (client-side search + total row), Khoản hỗ trợ form, both Chưa ánh xạ pages (friendly alert box, search on Phát's postman-code list), and the login page (restored a focus ring that had been accidentally removed). PO UI Check Required: Yes - verified working in a real browser session by Claude Code across multiple breakpoints, but formal Product Owner visual review/PASS has not happened yet.` |

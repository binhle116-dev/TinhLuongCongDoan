...
| Employee Data Status | `159 real employees imported 2026-08-27 (DEC-010) across 7 post offices + 1 management unit (TTVH). 115/159 have postman_code (backfilled from March 2026 data by Ma HRM match); 44 do not yet. Recomputing 2026-08-26's single real data file against this roster now resolves 96 employees with a nonzero provisional piece-rate total (sum 20,386,369.20d for that one day) - a real, non-zero, plausible end-to-end result for the first time. 56 distinct postman codes in real raw data still have no employee match (stale backfill source and/or CTV staff outside this roster) - visible via /bao-cao/chua-anh-xa/.` |

...
| UI Status | `Sidebar-based redesign shipped 2026-08-27 (DEC-015), restructured 2026-08-28 (DEC-020, DEC-021): "Tổng quan" and "Nhân sự" pulled out of Công đoạn Phát into their own top-level sections (both shared across all công đoạn now, live in the `core` app). New Tổng quan (login landing page) shows Lương per công đoạn side by side with Năm/Tháng + Bưu cục filters. Nhân viên page (DEC-021) now groups strictly by 1 buu cuc at a time via a selector (default: first in scope) instead of mixing all offices in one list. Found + fixed a real RBAC bug along the way: payroll_detail's buu cuc dropdown would have crashed for any real Trưởng bưu cục (never caught since all testing used the admin account) - see `scope_post_office_choices()`. PO UI Check Required: Yes - verified working in a real browser session by Claude Code, but formal Product Owner visual review/PASS has not happened yet.` |

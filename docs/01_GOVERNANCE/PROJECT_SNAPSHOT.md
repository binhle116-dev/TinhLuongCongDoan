...
| UI Status | `Sidebar-based redesign shipped 2026-08-27 (DEC-015), restructured 2026-08-28 (DEC-020, DEC-021): "Tổng quan" and "Nhân sự" pulled out of Công đoạn Phát into their own top-level sections (both shared across all công đoạn now, live in the `core` app). New Tổng quan (login landing page) shows Lương per công đoạn side by side with Năm/Tháng + Bưu cục filters. Nhân viên page (DEC-021) now groups strictly by 1 buu cuc at a time via a selector (default: first in scope) instead of mixing all offices in one list. Found + fixed a real RBAC bug along the way: payroll_detail's buu cuc dropdown would have crashed for any real Trưởng bưu cục (never caught since all testing used the admin account) - see `scope_post_office_choices()`. Khai thac dashboard fully redesigned 2026-08-28 (DEC-024): real KPI trend comparisons, vi-VN number formatting, service-mix donut, sortable/filterable/paginated daily table with mobile card fallback, sidebar unified to inline-SVG icons with desktop collapse + mobile drawer. Surfaced and fixed 2 real bugs along the way (PostOffice RBAC scoping in khaithac views, banker's-rounding in the new number filter). PO UI Check Required: Yes - verified working in a real browser session by Claude Code across 4 breakpoints, but formal Product Owner visual review/PASS has not happened yet.` |
